...
async function changeOrderNumbersToThoseFromSelects(mainContaner) {
    ...
    changingOrderNumberErrorContainer.setAttribute('id', 'course-all-modules-sections-changing-order-number-error-container');
    changingOrderNumberErrorContainer.setAttribute('class', 'error');
    mainContaner.appendChild(changingOrderNumberErrorContainer);

    let selectsElements = document.querySelectorAll(".course-all-modules-sections-module-order-selects");
    ...
    let selectsValues = [...selectsElements].map(select => Number(select.value));


    let selectedValuesOccurMoreThanOnce = checkIfElementOccursInArrayMoreThanOnce(selectsValues);
    if (selectedValuesOccurMoreThanOnce) {
        changingOrderNumberErrorContainer.textContent = "Każdy element musi mieć niepowtarzalny numer porządkowy!";
        return false;
    }
    changingOrderNumberErrorContainer.textContent = "";
    let orderNumbersChangedCorrectly = await changleModulesOrderNumbers(selectElementsIdAndOrderNumbers);
    if (!orderNumbersChangedCorrectly) changingOrderNumberErrorContainer.textContent = "Błąd serwera. Nie zmieniono kolejności modułów lub zmieniono częściowo";
...
async function changleModulesOrderNumbers(selectsArrayOfDicts) {
    ...
    for (let i = 0; i < selectsArrayOfDicts.length; i++) {
        let updated = await editModule(selectsArrayOfDicts[i]["id"], "order_number", selectsArrayOfDicts[i]["order_number"]);
...
async function displayTeachers(courseJsonData) {
    let divForDisplayingTeachers = id("courses-all-modules-sections-courses-teachers");
    ...
    for (let i in teachers) {
        ...
        // console.log(i);
        if (i != teachers.length - 1) divForDisplayingTeachers.appendChild(comma);
    }
}
async function changeOrderNumbers(modulesDictionary) {
    let orderNumbers = Object.keys(modulesDictionary).map(element => Number(element));
    orderNumbers.sort(function (a, b) { return a > b ? 1 : -1 });
    let newOrderNumbers = [];
    ...
    arrayOfModulesOrderNumbersAndIds.sort(function (a, b) { return a.old_order_number - b.old_order_number });
    for (let i = 0; i < arrayOfModulesOrderNumbersAndIds.length; i++) {
        arrayOfModulesOrderNumbersAndIds[i]["new_order_number"] = newOrderNumbers[i];
        await editModule(arrayOfModulesOrderNumbersAndIds[i]["module_id"], "order_number", arrayOfModulesOrderNumbersAndIds[i]["new_order_number"]);
    }
    window.location.reload();
...
async function displayManyModules(course) {
    ...
    for (let key in modulesAssignedToThisCourse) {
        ...
        await displayOneModule(modulesAssignedToThisCourse[key], modulesContainer, newOrderNumbers);
    }
    return 1;
...
async function displayOneModule(moduleDict, allModulesContainer, newOrderNumbers) {
    ...
    moduleNameElement.setAttribute('id', `course-all-modules-sections-module-${moduleDict["id"]}-name`);
    ...
    moduleDescriptionElement.setAttribute('id', `course-all-modules-sections-module-${moduleDict["id"]}-description`);
    moduleDescriptionElement.textContent = moduleDict["description"] != "" ? moduleDict["description"] : "brak opisu";
    ...
    buttonToChangeModuleNameOrDescription.setAttribute('id', `course-all-modules-sections-button-to-edit-module-${moduleDict["id"]}`);
    buttonToChangeModuleNameOrDescription.textContent = "Zmień nazwę lub opis modułu";
    divForButtonToChangeModuleNameOrDescription.appendChild(buttonToChangeModuleNameOrDescription);
    mainContainerForModule.appendChild(divForButtonToChangeModuleNameOrDescription);


    let buttonToAddSectionToModule = document.createElement('button');
    buttonToAddSectionToModule.setAttribute('id', `course-all-modules-sections-button-to-add-section-to-module-number-${moduleDict["id"]}`);
    ...
    editModuleErrorContainer.setAttribute('id', `course-all-modules-sections-module-${moduleDict["id"]}-error-container`);
    editModuleErrorContainer.setAttribute('class', `error`);
    mainContainerForModule.appendChild(editModuleErrorContainer);



    buttonToChangeModuleNameOrDescription.addEventListener('click', async function (e) {
        e.preventDefault();
        let editModuleDiv = id(`course-all-modules-sections-module-${moduleDict["id"]}-edit-div`);
        if (!editModuleDiv) {
            await editModuleManager(moduleDict, divForButtonToChangeModuleNameOrDescription);
        }
...
async function displayModuleSections(moduleId, mainContainerForModule) {
    ...
    let sectionsAssignedToThisModule = await getSectionsAssignedToTheModule(moduleId, errorContainerForDisplayingSections);

    console.log("MODULE: ", moduleId);
    console.log(sectionsAssignedToThisModule);
    let containerForAllModuleSections = document.createElement("div");
    containerForAllModuleSections.setAttribute("id", `course-all-modules-sections-module-${moduleId}-all-sections-div`);
...
function getLastElementOrderNumber(elementsAssignedToThisSection) {
    ...
    for (let i = 0; i < elementsAssignedToThisSection.length; i++) {
        console.log("order number: ", elementsAssignedToThisSection[i]["order_number"]);
        if (Number(elementsAssignedToThisSection[i]["order_number"]) > maxNumber) {
            maxNumber = Number(elementsAssignedToThisSection[i]["order_number"]);
        }
    }
    return maxNumber;
...
async function displaySectionElements(sectionDict, errorContainerForDisplayingSections, allSectionsDiv) {
    ...
    let lastElementOrderNumber = getLastElementOrderNumber(elementsAssignedToThisSection);
    ...
    sectionMainContainer.setAttribute('id', `course-all-modules-sections-section-${sectionDict["id"]}-main-container`);
    sectionMainContainer.setAttribute('class', `course-all-modules-sections-section-main-containers`);

    let sectionNameElement = document.createElement('div');
    sectionNameElement.setAttribute('id', `course-all-modules-sections-section-${sectionDict["id"]}-name-div`);
    sectionNameElement.setAttribute('class', `course-all-modules-sections-section-name-divs`);
    ...
    deleteSectionButton.addEventListener('click', async function (e) {
        e.preventDefault();
        let confirmed = window.confirm(`Czy na pewno chcesz usunąć sekcję ${sectionDict["name"]}?`);
        if (confirmed) {
            let elementDeletedCorrectly = await deleteSection(sectionDict["id"]);
            if (elementDeletedCorrectly) { alert('Usunięto sekcje'); window.location.reload(); }
    ...
    divForInputs.setAttribute('class', `course-all-modules-sections-section-divs-for-inputs`);
    sectionMainContainer.appendChild(divForInputs);



    let addSectionTextElementButton = document.createElement('button');
    addSectionTextElementButton.setAttribute('id', `course-all-modules-sections-section-${sectionDict["id"]}-button-to-add-text-element`);
    addSectionTextElementButton.setAttribute('class', `course-all-modules-sections-section-buttons-to-add-text-elements`);
    addSectionTextElementButton.textContent = "Dodaj tekst";
    sectionMainContainer.appendChild(addSectionTextElementButton);
    addSectionTextElementButton.addEventListener('click', async function (e) {
        e.preventDefault();
        let inputTextPlace = id("course-all-modules-sections-add-text-element-input");
        if (!inputTextPlace) {
            await addTextElementToSection(sectionDict["id"], divForInputs, newElementOrderNumber);
        }

    });
    let addSectionFileElementButton = document.createElement('button');
    addSectionFileElementButton.setAttribute('id', `course-all-modules-sections-section-${sectionDict["id"]}-button-to-add-file-element`);
    addSectionFileElementButton.setAttribute('class', `course-all-modules-sections-section-buttons-to-add-file-elements`);
    addSectionFileElementButton.textContent = "Dodaj plik";
    sectionMainContainer.appendChild(addSectionFileElementButton);
    addSectionFileElementButton.addEventListener('click', async function (e) {
        e.preventDefault();
        let inputFilePlace = id("course-all-modules-sections-add-file-element-input");
        if (!inputFilePlace) {
    ...
    for (let i = 0; i < elementsAssignedToThisSection.length; i++) {
        ...
        elementMainContainer.setAttribute('id', `course-all-modules-sections-section-${element["section"]}-${elementType}-element-${element["id"]}-main-container`);
        elementMainContainer.setAttribute('class', `course-all-modules-sections-section-elements-containers`);
        // elementMainContainer.textContent=element["id"]+elementType+element["section"];

        let elementContentDiv = document.createElement('div');
        elementContentDiv.setAttribute('id', `course-all-modules-sections-section-${element["section"]}-${elementType}-element-${element["id"]}-content`);
        elementContentDiv.setAttribute('class', `course-all-modules-sections-section-elements-content`);
        ...
        if (elementType == "file") {
            ...
            aHrefToDownloadFile.setAttribute('id', `course-all-modules-sections-section-${element["section"]}-${elementType}-element-${element["id"]}-button-to-download`);
            aHrefToDownloadFile.setAttribute('class', `course-all-modules-sections-section-elements-download-file btn btn-warning`);
            aHrefToDownloadFile.setAttribute('href', `${appAddress}/assets/${element["file"]}?download`)
        ...
        buttonToDeleteSectionElement.addEventListener('click', async function (e) {
            e.preventDefault();
            let confirmed = window.confirm(`Czy na pewno chcesz usunąć ten element?`);
            if (confirmed) {
                let elementDeletedCorrectly = await deleteSectionElementManager(element["id"], elementType);
                if (elementDeletedCorrectly) { alert('Usunięto element'); window.location.reload(); }
...
async function addTextElementToSection(sectionId, containerDivForInput, orderNumberForNewElement) {

    let divForInputPlace = document.createElement('div');
    divForInputPlace.setAttribute('id', `course-all-modules-sections-add-text-element-div-for-input`);
    divForInputPlace.setAttribute('class', 'course-all-modules-sections-add-text-element-divs-for-inputs');
    ...
    inputTextPlace.setAttribute('id', `course-all-modules-sections-add-text-element-input`);
    inputTextPlace.setAttribute('class', 'course-all-modules-sections-add-text-element-inputs');
    divForInputPlace.appendChild(inputTextPlace);

    let buttonToSaveText = document.createElement('button');
    buttonToSaveText.setAttribute('id', 'course-all-modules-sections-add-text-element-submit-button');
    ...
    buttonToSaveText.addEventListener('click', async function (e) {
        e.preventDefault();
        let textAddedCorrectly = await addTextElementToDatabase(sectionId, orderNumberForNewElement, inputTextPlace.value);
    ...
    buttonToRemoveInputPlace.setAttribute('id', 'course-all-modules-sections-add-text-element-remove-input-place-button');
...
async function addTextElementToDatabase(sectionId, orderNumberForNewElement, textInput) {
    let dataToPost = {
        "section": sectionId,
...
async function addFileElementToSection(sectionId, containerDivForInput, orderNumberForNewElement) {

    let divForInputPlace = document.createElement('div');
    divForInputPlace.setAttribute('id', `course-all-modules-sections-add-file-element-div-for-input`);
    divForInputPlace.setAttribute('class', 'course-all-modules-sections-add-file-element-divs-for-inputs');
    containerDivForInput.appendChild(divForInputPlace);

    let inputFilePlace = document.createElement('input');
    inputFilePlace.setAttribute('id', `course-all-modules-sections-add-file-element-input`);
    inputFilePlace.setAttribute('type', 'file');
    inputFilePlace.setAttribute('class', 'course-all-modules-sections-add-file-element-inputs');
    divForInputPlace.appendChild(inputFilePlace);

    let buttonToSaveFile = document.createElement('button');
    buttonToSaveFile.setAttribute('id', 'course-all-modules-sections-add-file-element-submit-button');
    ...
    buttonToSaveFile.addEventListener('click', async function (e) {
        ...
        let fileAddedCorrectly = await addFileElementManager(fileData, sectionId)
    ...
    let buttonToRemoveInputPlace = document.createElement('button');
    buttonToRemoveInputPlace.setAttribute('id', 'course-all-modules-sections-add-file-element-remove-input-place-button');
    buttonToRemoveInputPlace.setAttribute('class', 'btn btn-danger');
    buttonToRemoveInputPlace.textContent = "X";
    divForInputPlace.appendChild(buttonToRemoveInputPlace);
...
async function deleteSectionElementManager(elementId, elementType) {
    ...
    try {
        ...
        response = await fetch(`${appAddress}/items/${collectionName}/${elementId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem("access_token")}`
...
async function displayElementBasedOnItsType(element, elementContentDiv) {
    ...
    contentNameDiv.setAttribute('id', `course-all-modules-sections-section-${element["section"]}-element-${element["id"]}-content-name-div`);
    contentNameDiv.setAttribute('class', `course-all-modules-sections-section-elements-content-name-divs`);
    ...
    if (String(fileInfo["type"]).substring(0, 5) == "image") {
        imgElement.setAttribute("src", `${appAddress}/assets/${element["file"]}?key=actual-size`);
        elementContentDiv.appendChild(imgElement);
    }
    else {
        setImgSourceBasedOnFileTypeAndAddItToProperContainer(imgElement, fileInfo["type"], elementContentDiv);
    }
...
function setImgSourceBasedOnFileTypeAndAddItToProperContainer(imgElement, fileType, elementContentDiv) {
    let iconSource = "";
    switch (fileType) {
        ...
        case 'application/vnd.openxmlformats-officedocument.presentationml.presentation': //pliki pptx
            ...
            break;
        default:
            iconSource = "e0ab86b9-c4d9-4ffc-bad5-d3a845ce3991"; //inne
            break;

    }
    imgElement.setAttribute("src", `${appAddress}/assets/${iconSource}?key=icon-size`);
...
async function getElementsAssignedToThisSection(sectionId, containerForError) {
    ...
    if (allFileElements == null || allTextElements == null) {
        containerForError.textContent = "Wystąpił problem z pobraniem elementów należących do modułu";
        return null;
    }
    ...
    for (let i = 0; i < fileData.length; i++) {
        let item = fileData[i];
        // console.log(item);
        if (item["section"] == sectionId) elementsAssignedToThisSection.push(item);
    }
...
async function editModuleManager(moduleDict, mainContainerForModule) {
    ...
    editModuleDiv.setAttribute('id', `course-all-modules-sections-module-${moduleDict["id"]}-edit-div`);


    mainContainerForModule.appendChild(editModuleDiv);

    let labelForNameInput = document.createElement('label');
    labelForNameInput.setAttribute('for', 'course-all-modules-sections-module-name-input');
    labelForNameInput.textContent = "Nazwa modułu";

    let moduleNameInput = document.createElement('input');
    moduleNameInput.setAttribute('id', 'course-all-modules-sections-module-name-input');
    moduleNameInput.setAttribute('placeholder', `${moduleDict["name"]}`);

    let labelForDescriptionInput = document.createElement('label');
    labelForDescriptionInput.setAttribute('for', 'course-all-modules-sections-module-description-input');
    labelForDescriptionInput.textContent = "Opis modułu";

    let moduleDescriptionInput = document.createElement('input');
    moduleDescriptionInput.setAttribute('id', 'course-all-modules-sections-module-description-input');
    moduleDescriptionInput.setAttribute('placeholder', `${String(moduleDict["description"]).substring(0, 25)}...`);

    let submitButton = document.createElement('button');
    submitButton.setAttribute('id', 'course-all-modules-sections-edit-module-submit-button');
    submitButton.textContent = "Zapisz zmiany";

    let buttonDeteleOptionToEditModule = document.createElement('button');
    buttonDeteleOptionToEditModule.setAttribute('id', 'course-all-modules-sections-edit-module-button-detele-option-to-edit-module');
    ...
    submitButton.addEventListener('click', async function (e) {
        e.preventDefault();
        let moduleEdittedSuccessfully = await editModuleNameOrDesrciptionInDatabase(moduleDict["id"], moduleNameInput.value, moduleDescriptionInput.value, moduleDict);
        console.log(moduleEdittedSuccessfully);
        if (moduleEdittedSuccessfully["nameEditted"] == false || moduleEdittedSuccessfully["descriptionEdited"] == false) {
...
async function editModuleNameOrDesrciptionInDatabase(moduleId, newName, newDescription, moduleDict) {
    ...
    if (newDescription != moduleDict["description"] && newDescription != "") {
        descriptionEdittedSuccessfully = await editModule(moduleId, "description", newDescription);
    }
    return {
