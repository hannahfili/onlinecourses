    ...
    if (!courses.ok) {
        ...
        for (var i = 0; i < coursesJson.data.length; i++) {
            (async function (index) {
                ...
                if (course["maximum_students_count"]==course["number_of_students_attending_course"]) return;
                ...
                if(teacherLoggedIn) {teacherIsAssignedToCourse=await checkIfTeacherIsAssignedToCourse(course.id, localStorage.getItem('loggedInUserId'));
                ...
                if (studentLoggedIn) {
                    ...
                    buttonSignUp.addEventListener('click', async function (e) {
                        ...
                        if(studentIsSignedToCourse){
                            ...
                            if(removed) {alert('Pomyślnie wypisano Cię do kursu');
                        window.location.reload();}
